...
describe('sortByPubDate', () => {
  it('sorts posts by pubDate descending', () => {
    ...
    sortByPubDate(posts);
    expect(posts[0].data.pubDate.toISOString()).toBe(
      '2025-06-15T00:00:00.000Z',
    );
    expect(posts[1].data.pubDate.toISOString()).toBe(
      '2024-12-31T00:00:00.000Z',
    );
    expect(posts[2].data.pubDate.toISOString()).toBe(
    ...
    getCollectionMock.mockImplementation(
      async (_collection: string, filter?: (entry: any) => boolean) => {
        const entries = [
          {
            id: '123',
            data: { pubDate: new Date(), draft: false },
            body: 'text',
          },
          {
            id: 'hello-world',
    ...
    getCollectionMock.mockImplementation(
      async (_collection: string, filter?: (entry: any) => boolean) => {
        const entries = [
          ...
          {
            id: 'post-b',
            data: { pubDate: new Date(), draft: true },
            body: '',
          },
        ];
        return filter ? entries.filter(filter) : entries;
    ...
    getCollectionMock.mockImplementation(
      async (_collection: string, filter?: (entry: any) => boolean) => {
        const entries = [
          ...
          {
            id: 'new',
            data: { pubDate: new Date('2025-01-01'), draft: false },
            body: '',
          },
        ];
        return filter ? entries.filter(filter) : entries;
